                ...
                for code_mod, data in PROTOCOLE_BARLOW.items():
                    ...
                    with c_lock:
                        if code_mod in progression:
                            # Le module est actuellement OUVERT. 
                            # Le bouton doit permettre de le BLOQUER.
                            # On affiche un cadenas OUVERT pour signifier "C'est ouvert".
                            if st.button("🔓", key=f"btn_lock_{patient_sel}_{code_mod}", help="Module accessible. Cliquer pour BLOQUER."):
                                progression.remove(code_mod)
                                sauvegarder_progression(patient_sel, progression)
                                st.rerun()
                        else:
                            # Le module est actuellement FERMÉ.
                            # Le bouton doit permettre de l'OUVRIR.
                            # On affiche un cadenas FERMÉ (rouge) pour signifier "C'est fermé".
                            if st.button("🔒", key=f"btn_unlock_{patient_sel}_{code_mod}", type="primary", help="Module verrouillé. Cliquer pour DÉBLOQUER."):
